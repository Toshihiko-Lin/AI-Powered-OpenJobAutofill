...
const STRUCTURED_RESUME_SECTIONS = (globalThis.OJAF_PROFILE_SCHEMA || []).map((section) => ({
  ...section,
  fields: profileFields(section.fields)
}));
...
async function refreshLearningSummary() {
  ...
  try {
    ...
    fields.clearLearningButton.textContent = count > 0 ? `清空暂存的修改记录（${count} 条）` : "清空暂存的修改记录";
    fields.clearLearningButton.disabled = count === 0;
  } catch {
    fields.clearLearningButton.disabled = true;
  }
}

async function clearLearningRecords() {
  const confirmed = window.confirm("这会删除所有页面暂存、尚未写入资料库的修改记录，是否继续？");
  if (!confirmed) {
    return;
  }
  try {
    const result = await sendRuntimeMessage({ type: "OJAF_DISCARD_LEARNING_RECORDS", payload: { all: true } });
    setPreferencesFeedback(`已清空 ${result?.removed || 0} 条暂存记录。`, "saved");
    await refreshLearningSummary();
  } catch (error) {
    setPreferencesFeedback(`清空失败：${error.message}`, "error");
  }
}

function setPreferencesFeedback(message, state = "") {
  if (!fields.preferencesFeedback) {
    return;
  }
  fields.preferencesFeedback.textContent = message;
  fields.preferencesFeedback.classList.toggle("is-saved", state === "saved");
  fields.preferencesFeedback.classList.toggle("error", state === "error");
}
