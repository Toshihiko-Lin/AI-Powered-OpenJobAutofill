...
隐私边界很明确：AI 请求只包含当前页面字段和本机资料字段名称，不包含你的姓名、手机号、身份证号、经历内容等具体资料值。真正的资料取值和填写都在本机完成。"更新资料库"里的 AI 归类同样只发送字段名称。

两个例外，都由你控制：

- "上传简历自动解析"必须把简历全文发给模型才能拆分字段，因此每次点击都会弹出确认，并明确说明发送目标。
- "AI 选项匹配"（偏好设置里默认关闭）：下拉/单选在本地匹配不到选项时（未婚 vs 单身、全日制 vs 统招），把该字段的资料值和页面选项列表发给模型选一个。只对学历、学位、学习形式、政治面貌、婚姻状况、民族、城市、是否类等低敏感字段生效，姓名、电话、证件号、地址、学号、薪资、家庭成员永远不发送。
